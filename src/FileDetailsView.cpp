...
namespace
{
     ...
     **/
    void setLabelLimited( QLabel * label, const QString & text, int limit )
    {
     ...
     **/
    void updatePkgInfo( const Ui::FileDetailsView * ui, const QString & path, int labelLimit )
    {
	// logDebug() << "Updating pkg info for " << path << Qt::endl;

	const QString pkg = PkgQuery::owningPkg( path );
	setLabelLimited( ui->filePackageLabel, pkg, labelLimit );
     ...
     * item.
     **/
    void showFileInfo( const Ui::FileDetailsView * ui, const FileInfo * file, int labelLimit )
    {
	const bool isSpecial = file->isSpecial();
	const bool isSymLink = file->isSymLink();

	setLabelLimited(ui->fileNameLabel, file->baseName(), labelLimit );
	...
	if ( isSymLink )
	{
	    ...
	    setLabelLimited( ui->fileLinkLabel, shortTarget, labelLimit ); // don't set tooltip yet
	...
	if ( PkgQuery::foundSupportedPkgManager() )
	{
		...
		const auto payload = [ ui, url, labelLimit ]() { updatePkgInfo( ui, url, labelLimit ); };
     ...
     **/
    void showDirInfo( const Ui::FileDetailsView * ui, DirInfo * dir, int labelLimit )
    {
	// logDebug() << "Showing dir details about " << dir << Qt::endl;

	const QString name = dir->isPseudoDir() ? dir->name() : ( dir->baseName() % '/' );
	setLabelLimited( ui->dirNameLabel, name, labelLimit );
     ...
     **/
    void showPkgInfo( const Ui::FileDetailsView * ui, PkgInfo * pkg, int labelLimit )
    {
	// logDebug() << "Showing pkg details about " << pkg << Qt::endl;

	setLabelLimited( ui->pkgNameLabel, pkg->name(), labelLimit );
...
void FileDetailsView::showDetails( const FileInfoSet & selectedItems )
{
    ...
    showFileInfoSet( ui(), selectedItems.normalized() );
...
void FileDetailsView::showDetails( FileInfo * file )
{
	...
	{
	    showPkgInfo( ui(), file->toPkgInfo(), _labelLimit );
	    setCurrentPage( _ui->pkgDetailsPage );
	}
    }
    else if ( file->isDirInfo() )
    {
	showDirInfo( ui(), file->toDirInfo(), _labelLimit );
	setCurrentPage( _ui->dirDetailsPage );
    }
    else
    {
	// logDebug() << "Showing file details about " << file << Qt::endl;
	showFilePkgInfo( ui(), _pkgUpdateTimer, file, _labelLimit );
	showFileInfo( ui(), file, _labelLimit );
