...
namespace QDirStat
{
     ...
     * - percentiles with the boundaries between each percentile;
     * - percentileCounts with the cumulative sum of all entries up to percentile;
     * - percentileSums with the cumulative sum of size values up to a percentile;
...
//	    { return std::accumulate( _bucketCounts.cbegin(), _bucketCounts.cend(), 0 ); }
	 ...
	 * indexes or lists.
	 **/
	PercentileValue percentileCountDiff( int endIndex, int startIndex ) const
	 ...
	 *
	 * Run calculatePercentiles() and then fillBuckets() before calling
	 * this function. If the buckets list is empty then it will return
	 * 0.
	 **/
//	PercentileBoundary bucketWidth() const
//	    { return _bucketCounts.isEmpty() ? 0 : ( _bucketsEnd - _bucketsStart ) / _bucketCounts.size(); }
	...
	PercentileList _percentileCounts;
	PercentileList _percentileSums;

	Buckets            _buckets;
	BucketList         _bucketCounts;
